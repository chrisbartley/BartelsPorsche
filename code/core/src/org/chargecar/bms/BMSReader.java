...
class BMSReader extends StreamingSerialPortReader<BMSEvent>
   ...
   BMSReader(final String serialPortName, final int numBatteries)
      {
   ...
   BMSReader(final SerialIOManager serialIOManager, final int numBatteries)
      {
   ...
   protected void processSentence(final Date timestamp, final byte[] sentenceBytes)
      ...
      if (sentenceBytes != null)
         ...
         if (LOG.isTraceEnabled())
            {
            LOG.trace("BMSReader.processSentence(): sentenceBytes.length=[" + sentenceBytes.length + "]");
            }
   ...
   private BMSEvent createBMSEvent(final Date timestamp,
      ...
      final double[] cellVoltages = new double[numBatteries];
      for (int i = 0; i < Math.min(cellDataVoltages.length, numBatteries); i++)
         {
      ...
      final int[] cellTemperatures = new int[numBatteries];
      for (int i = 0; i < Math.min(cellDataTemperatures.length, numBatteries); i++)
         {
      ...
      final int[] cellResistances = new int[numBatteries];
      for (int i = 0; i < Math.min(cellDataResistances.length, numBatteries); i++)
         {
